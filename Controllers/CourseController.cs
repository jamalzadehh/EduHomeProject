...
namespace EduHome.Controllers
{
    public class CourseController : Controller
    {
        ...
        public IActionResult RequestCourse(int courseId,Request request)
        {
            ...
            if (_context.Users.Include(x=>x.Requests).FirstOrDefault(x=>x.Id==user.Id).Requests.Any(x=>x.CourseId==courseId) && _context.Users.Include(x => x.Requests).FirstOrDefault(x => x.Id == user.Id).Requests.FirstOrDefault(x=>x.CourseId==courseId).Status!= EduHomeProject.Enums.RequestStatus.UserReject)
            {
                return RedirectToAction("index");
            }
            ...
            request.Status = EduHomeProject.Enums.RequestStatus.Pending;
            request.Id = 0;
        ...
        public IActionResult RejectCourse(int courseId,string userId)
        {
            if (_context.Requests.FirstOrDefault(x => x.CourseId == courseId && x.AppUserId == userId) !=null)
            {
                ...
                _context.Requests.FirstOrDefault(x => x.CourseId == courseId && x.AppUserId == userId && x.RequestDate == date).Status = EduHomeProject.Enums.RequestStatus.UserReject;
